...
def predict_frames(graph, np_frames, labels_index):
    with graph.as_default():
        with tf.Session(graph=graph) as sess:
            with open(PREDICTION_OUTPUT, "w") as fl:
                ...
                for frame in np_frames:
                    ...
                    for i, cls in enumerate(d_classes[0]):
                        ...
                        frame_desc["object"] = labels_index[cls]
                        frame_desc["bounding_box"] = boxes[0][i]
                        frame_desc["confidence"] = scores[0][i]
                        fl.write(json.dumps(frame_desc) + "\n")
...
if __name__ == "__main__":
    ...
    graph = load_graph()
    arr = np.load("./cache/ETH-Bahnhof.npz")["arr"]
